...
const listProduct = async (req, res) => {
  try {
    const { limit = 10, q = "", ...filter } = req.query;
    // const {limit:pizzalimit = 10} = req.query
    // const limitedPizza = await productModel.find().limit(pizzalimit);
    // const limitedPizza = await productModel.find({title:{
    //   $regex:q
    // }}).limit(limit);
    const limitedPizza = await productModel.find(filter).limit(limit);
    res.json({
      status: "Success",
      data: limitedPizza,
      result: limitedPizza.length,
    });
  } catch (err) {
    console.log(err);
    console.log("Internal error in listing product");
    console.log(Error.message);
    res.status(500).json({
      succuss: false,
      message: "Internal server error",
